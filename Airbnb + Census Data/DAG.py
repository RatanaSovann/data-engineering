...
CENSUS = AIRFLOW_DATA + "/Census LGA/"
LGA = AIRFLOW_DATA + "/NSW_LGA/"
...
def import_load_listings_func(**kwargs):
    ...
    filelist = sorted([k for k in os.listdir(LISTINGS) if k.endswith('.csv')])
    ...
    logging.info("CSV files", filelist)
    
    for fname in filelist:
        # Generate dataframe by reading the CSV file
        df = pd.read_csv(os.path.join(LISTINGS, fname))
        if len(df) > 0:
            ...
            insert_sql = """
                        INSERT INTO bronze.listings (LISTING_ID, SCRAPE_ID, SCRAPED_DATE, HOST_ID, HOST_NAME, HOST_SINCE, 
                        HOST_IS_SUPERHOST, HOST_NEIGHBOURHOOD, LISTING_NEIGHBOURHOOD, PROPERTY_TYPE, 
                        ROOM_TYPE, ACCOMMODATES, PRICE, HAS_AVAILABILITY, AVAILABILITY_30, 
                        NUMBER_OF_REVIEWS, REVIEW_SCORES_RATING, REVIEW_SCORES_ACCURACY, 
                        REVIEW_SCORES_CLEANLINESS, REVIEW_SCORES_CHECKIN, REVIEW_SCORES_COMMUNICATION, 
            ...
            execute_values(cursor_ps, insert_sql, values, page_size=len(df))
            conn_ps.commit()
...
def import_load_census_g01_func(**kwargs):
    ...
    census_g01_file_path = CENSUS + '2016Census_G01_NSW_LGA.csv'
    ...
    if len(df) > 0:
        ...
        insert_sql = """
                    INSERT INTO bronze.census_g01 (LGA_CODE_2016, Tot_P_M, Tot_P_F, Tot_P_P, Age_0_4_yr_M, Age_0_4_yr_F, 
                    Age_0_4_yr_P, Age_5_14_yr_M, Age_5_14_yr_F, Age_5_14_yr_P, Age_15_19_yr_M, 
                    Age_15_19_yr_F, Age_15_19_yr_P, Age_20_24_yr_M, Age_20_24_yr_F, Age_20_24_yr_P, 
                    Age_25_34_yr_M, Age_25_34_yr_F, Age_25_34_yr_P, Age_35_44_yr_M, Age_35_44_yr_F, 
                    Age_35_44_yr_P, Age_45_54_yr_M, Age_45_54_yr_F, Age_45_54_yr_P, Age_55_64_yr_M, 
...
def import_load_census_g02_func(**kwargs):
    ...
    # Check if the file exists
    census_g02_file_path = CENSUS + '2016Census_G02_NSW_LGA.csv'
...
def import_load_lga_code_func(**kwargs):
    ...
    if len(df) > 0:
        ...
        insert_sql = """
                    INSERT INTO bronze.nsw_lga_code(LGA_CODE, LGA_NAME)
                    VALUES %s
                    """
        execute_values(cursor_ps, insert_sql, values, page_size=len(df))
        conn_ps.commit()
...
def import_load_lga_suburb_func(**kwargs):
    ...
    if len(df) > 0:
        ...
        insert_sql = """
                    INSERT INTO bronze.nsw_lga_suburb(LGA_NAME, SUBURB_NAME)
                    VALUES %s
                    """
...
# Task Dependencies
[import_load_census_g01_task, import_load_census_g02_task, import_load_lga_code_task, 
 import_load_lga_suburb_task] >> import_load_listings_task 